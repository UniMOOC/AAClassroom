...
def generate_units_select():
    unit_choices = [(-1, '-- None --')]
    for unit in course.get_units():
        if (unit.type == verify.UNIT_TYPE_UNIT
                and unit.unit_id != this_unit_id):
            unit_choices.append((unit.unit_id, unit.title))
    return unit_choices
...
def generate_common_schema(title):
    ...
    # UM_CODE_START
    common.add_property(SchemaField(
        'unit_image', 'UM Unit Image', 'string', optional=True,
        description='URL for the image of this unit.'))
    common.add_property(SchemaField(
        'unit_parent', 'UM Unit Parent', 'integer', optional=True,
        description='Parent unit this unit belongs to'))
    common.add_property(SchemaField(
        'docs', 'UM Unit Documentation', 'html', optional=True,
        description='JSON with the docs for this unit.'))
    # UM_CODE_END
    return common
...
class UnitTools(object):
    ...
    def _apply_updates_common(self, unit, updated_unit_dict, errors):
        ...
        # UM_CODE_START
        unit.unit_image = updated_unit_dict.get('unit_image')
        unit.unit_parent = updated_unit_dict.get('unit_parent')
        unit.docs = updated_unit_dict.get('docs')
        # UM_CODE_END
    ...
    def _apply_updates_to_assessment(self, unit, updated_unit_dict, errors):
        ...
        # UM_CODE_START
        unit.badge = entity_dict.get('badge')
        unit.min_score = entity_dict.get('min_score')
        unit.required_asms = entity_dict.get('required_asms')
        # UM_CODE_END
    ...
    def _apply_updates_to_unit(self, unit, updated_unit_dict, errors):
        ...
        # UM_CODE_START
        unit.certificate = updated_unit_dict['certificate']
        # UM_CODE_END
    ...
    def _assessment_to_dict(self, unit, keys=None):
        ...
        unit_common.update({
            ...
            # UM_CODE_START
            'badge': unit.badge,
            'required_asms': unit.required_asms,
...
def generate_unit_schema():
    ...
    # UM_CODE_START
    schema.add_property(SchemaField(
        'certificate', 'UM Unit Certificate', 'string', optional=True,
        description='id of the certificate for this unit.'))
    # UM_CODE_END
    ...
    def get_annotations_dict(cls, course, this_unit_id):
        ...
        for unit in course.get_units():
            if (unit.type == verify.UNIT_TYPE_UNIT
                    and unit.unit_id != this_unit_id):
                unit_choices.append((unit.unit_id, unit.title))
        schema.get_property('unit_parent').set_select_data(unit_choices)
        # UM_CODE_END
...
def create_assessment_registry():
    ...
    # UM_CODE_START
    course_opts.add_property(
        SchemaField('badge', 'UM Assessment badge', 'string', optional=True,
            description='Id of the badge for this assessment.'))
    course_opts.add_property(
        SchemaField('min_score', 'UM Minimum score', 'string', optional=True,
            description='Minimum score to pass the assessment.'))
    course_opts.add_property(
        SchemaField('required_asms', 'UM Required Assessments', 'string', optional=True,
            description='List of asm_ids that are a requisite to attempt this assessment.'))
    # UM_CODE_END
