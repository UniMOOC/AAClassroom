...
.controller('ExtraAttemptsCtrl',['$scope', '$q', '$routeParams', '$location', 'Resources','Constants','UserSvc', 'UIHandler', 'OpenBadgesSvc',
  function ($scope, $q, $routeParams, $location, Resources, Constants, UserSvc, UIHandler, OpenBadgesSvc) {
    ...
    UserSvc.WhenLoad().then(function(){
        Resources.ExtraAttempts.get({'url_hash': $routeParams.url_hash}, function(assessment){

            Resources.promises.canGoMain.promise.then(function(){

                assessment.preamble = 'Una vez finalices el examen selecciona “Enviar respuestas”. Si superas el examen (con un 80% de aciertos), no tendrás que volver a realizarlo ya que las respuestas quedarán registradas. Recuerda que hoy solo tienes <strong>un intento</strong> para aprobar el examen de este módulo.';
                if (! $scope.isLastAsmDate(assessment)) {
                    assessment.preamble += ' En caso de no superar el examen, una vez que acabes todo el Curso Especializado de Marketing Digital, tendrás otra oportunidad para conseguirlo. ¡Mucha suerte!';
                }

                $scope.assessment = assessment;
                $scope.assessmentCopy = assessment;
                if (window.localStorage.getItem(assessment.unit_id)) {
                    $scope.assessment.questions = JSON.parse(window.localStorage.getItem(assessment.unit_id));
                } else {
                    $scope.assessment.questions = $scope.Shuffle($scope.assessment.questions);
                }
                var asm         = $scope.FindAsmById(assessment.unit_id);
    ...
    $scope.FindAsmById = function(id){
        ...
        if ($scope.$parent.course.finalAsm.id === id) {
    ...
    $scope.Mark = function(){
        ...
        angular.forEach($scope.assessment.questions, function(obj){
        ...
        if (questionsLeft) {
            ...
            modal.result.then(function() {
                $scope.SendAnswers($routeParams.url_hash, answers);
            }, function() {
    ...
    $scope.SendAnswers = function(urlHash, answers) {
        Resources.ExtraAttempts.save({'url_hash': urlHash, 'answers': answers}, function(response){
            $scope.resetQuestions($scope.assessment.unit_id);
            var header = null, msg = null, type = null;
            // var header = 'Aprobado', msg = '', type = 'success', feedback = '';
            if (response.score >= $scope.assessment.min_score) {
                var badgesIssued = [];
                for (var badgeIndex in response.badge) {
                    var badgeId = response.badge[badgeIndex];
                    badgesIssued.push(OpenBadgesSvc.get({'badge_id': badgeId}).$promise);
                }

                $q.all(badgesIssued).then(function(data){
                    header = 'Aprobado';
                    type = 'success';
                    msg = {
                      main : '',
    ...
    $scope.change = function(question, value) {
        ...
        window.localStorage.setItem($scope.assessment.unit_id, JSON.stringify($scope.assessment.questions));
      };
    ...
    $scope.isLastAsmDate = function(assessment) {
        var asmUnit = $scope.FindAsmById(assessment.unit_id);
        return $scope.FindDate(asmUnit) === asmUnit.dates[asmUnit.dates.length - 1];
    };
  }]);
